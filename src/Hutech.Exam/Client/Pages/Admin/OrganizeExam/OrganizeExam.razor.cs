...
namespace Hutech.Exam.Client.Pages.Admin.OrganizeExam
{
    partial class OrganizeExam
    {
        ...
        private const string ALREADY_APPROVED = "Ca thi đã được duyệt rồi! Không thể duyệt lại được nữa.";
        ...
        {
            if(examSession.Approved)
            {
                Snackbar.Add(ALREADY_APPROVED, Severity.Error);
                return;
            }
        ...
        {
            var reason = await OpenAuditDialogAsync(KieuHanhDong.DuyetCaThi);
            if (reason != null && !reason.Canceled && reason.Data != null)
            {
                string jsonText = CreateActionHistory(KieuHanhDong.DuyetCaThi, "", reason.Data.ToString()!);
