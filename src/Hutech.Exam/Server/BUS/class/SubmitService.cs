        ...
        public async Task HandleSubmit(byte[] message)
        {
            ...
            try
            {
                ...
                if (submitRequest.IsLanDau) // chỉ thực hiện cho lần đầu
                {
                    ...
                    _logger.LogInformation("[SUCCESS] SVMa: {ma_sinh_vien} has score {diem}, total right answers {so_cau_dung} on {tong_so_cau}. Requeue!", submitRequest.MaSinhVien, diem, so_cau_dung, dapAns.Count);
                ...
                {
                    _logger.LogWarning("[CONFLICT] Data count mismatch: Redis {dataCount} vs Message {dapAnsCount} of maSV {maSV}", data.Count, length, submitRequest.MaSinhVien);
                    throw new Exception();
                }
